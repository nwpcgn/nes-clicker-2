<script>
	...
	import { sleep, Point, getRandInt } from '../../lib'
	...
	const onResize = () => {
		if (!element && !frame) alert('NO ELEMENT')
		if (element) {
...
{#if header}
	<header>
		<div class="flex items-center gap-4">
			<!-- <div>
			<label for="rx">X {x}</label>
			<input type="range" bind:value={x} min={xmin} max={xmax} id="rx" />
		</div>
		<div>
			<label for="ry">Y {y}</label>
			<input type="range" bind:value={y} min={ymin} max={ymax} id="ry" />
		</div> -->
			<div>
				<button on:click={setIn} class="btn">Toggle</button>
			</div>
		</div>
	</header>
{/if}
